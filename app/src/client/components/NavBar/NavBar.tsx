          ...
          >
            <div className='flex items-center gap-6'>
              ...
              >
                ...
                <span
                  className={cn('font-bold tracking-tight text-foreground transition-all duration-300', {
          ...
          <SheetHeader>
            <SheetTitle className='flex items-center'>
              <WaspRouterLink to={routes.LandingPageRoute.to}>
                <span className='sr-only'>Travel Cost Estimator</span>
                <LogoMark size='md' />
              </WaspRouterLink>
            </SheetTitle>
          </SheetHeader>
...
function renderNavigationItems(
  ...
  const menuStyles = cn({
    'block rounded-lg px-3 py-2 text-sm font-semibold leading-7 text-foreground hover:bg-accent hover:text-accent-foreground transition-colors uppercase tracking-[0.2em]':
      !!setMobileMenuOpen,
    'text-xs font-semibold leading-6 text-foreground duration-300 ease-in-out hover:text-primary transition-colors uppercase tracking-[0.25em]':
      !setMobileMenuOpen,
  });
...
const NavLogo = ({ isScrolled }: { isScrolled: boolean }) => (
  <div
    className={cn('relative flex items-center gap-2 transition-all duration-500', {
      'scale-100': !isScrolled,
      'scale-[0.92]': isScrolled,
    })}
  >
    <LogoMark size={isScrolled ? 'sm' : 'md'} />
  </div>
);
